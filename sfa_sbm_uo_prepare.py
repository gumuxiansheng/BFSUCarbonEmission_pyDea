...
def generate_sfa_input():
    for year in range(0, 11):
        ...
        table = xlrd.open_workbook(filename=file_url_base + 'RFrontierInputFiles/_sfa_in' + str(ac_year) + '.xls')
        table2 = read_excel(file_url_base + 'OSDEA_Output/out_dea' + str(ac_year) + '.xls').sheet_by_name('Targets')
        ...
        new_table.save(file_url_base + 'RFrontierInputFiles/_sfa_in' + str(ac_year) + '.xls')
...
def sfa_result_rearrange():
    ...
    for year in range(0, 11):
        ...
        f = open(file_url_base + 'RFrontierOutputFiles/_sfa_out' + str(ac_year) + '.txt', encoding='utf-8')  # 返回一个文件对象
        line = f.readline()
        row_num = 0
        ...
        while line:
            ...
            row_num += 1
        f.close()
    wbw.save(file_url_base + 'RFrontierOutputFiles/_sfa_out.xls')
    return
...
def sfa_result_xx_rearrange():
    ...
    for year in range(0, 11):
        ac_year = 2016 - year
        ws = wbw.add_sheet(str(ac_year), cell_overwrite_ok=True)
        f = open(file_url_base + 'RFrontierOutputFiles/_sfa_out_xx' + str(ac_year) + '.txt', encoding='utf-8')  # 返回一个文件对象
        line = f.readline()
        row_num = 0
        entering_status = 0
        while line:
            ...
            row_num += 1
        f.close()
    wbw.save(file_url_base + 'RFrontierOutputFiles/_sfa_out_xx.xls')
    return


def generate_3rd_dea_input_cal():
    wb = xlrd.open_workbook(filename=file_url_base + 'RFrontierOutputFiles/_sfa_out_xx.xls')
    wbw = xlwt.Workbook(encoding='utf-8', style_compression=0)
    for year in range(0, 11):
        ac_year = 2016 - year
        ...
        table2 = read_excel(file_url_base + 'RFrontierInputFiles/_sfa_in' + str(ac_year) + '.xls').sheet_by_index(0)
        max_co2 = 0
        max_capital = 0
        max_labour = 0
    ...
    wbw.save(file_url_base + 'RFrontierOutputFiles/_3rd_dea_input_cal.xls')  # 拟合值、残值信息
    return


def cal_vi():
    wbw = xlrd.open_workbook(filename=file_url_base + 'RFrontierOutputFiles/_sfa_out_xx.xls')  # 获取epsilon
    wbw2 = xlrd.open_workbook(filename=file_url_base + 'RFrontierOutputFiles/_sfa_out.xls')  # 获取sigma and lambda

    dst_file = xlrd.open_workbook(filename=file_url_base + 'RFrontierOutputFiles/_3rd_dea_input_cal.xls')  # 写入统一文件
    ...
    dst_file_new.save(file_url_base + 'RFrontierOutputFiles/_3rd_dea_input_cal.xls')
    return


def generate_adjusted_dea_input():
    wb_fitted = xlrd.open_workbook(filename=file_url_base + 'RFrontierOutputFiles/_3rd_dea_input_cal.xls')  # 拟合值，残值信息
    for year in range(0, 11):
        ac_year = 2016 - year
        wb = xlrd.open_workbook(filename=file_url_base + 'OSDEA_Input/_dea' + str(ac_year) + '.xls')
        ...
        dst_file_new.save(file_url_base + 'OSDEA_ThirdStageInput/_dea' + str(ac_year) + '.xls')
    return
